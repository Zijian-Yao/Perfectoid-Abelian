...
%%%%%%%%%%%%	
	...
	\begin{framed}
	\begin{question}
		Given a rigid group $G$ over a field $K$, when is there an adic space $G_\infty$ such that \[G_\infty \sim  \varprojlim_{[p]} G\] in the sense of \cite{SW}? If it exists, and $K$ is perfectoid, when is $G_\infty$ perfectoid?
	\end{question}
	\end{framed}
...
%%%%%%%%%%%%	
	...
	\[S(d) = \{(s_1,\dots,s_r)\in \mathbb R^r | |s_i|\leq d \}\]
	Choose $d$ small enough such that $S(2d)$ intersects $\Lambda$ only in $0\in \Lambda$. Choose
	 $q\in K$ such that $|q|=\exp(-d)$. We denote by $\mathcal B(q,q^{-1})$ the affinoid open rigid multi-annulus in $T$ centered at $1$ of radii $|q|< 1 < |q|^{-1}$ in every direction.
