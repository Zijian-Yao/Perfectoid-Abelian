...
%%%%%%%%%%%%	
	...
	With some work, the arguments in this section can be extended to any perfectoid base field. The Raynaud uniformisation of Theorem \ref{Raynaud uniformisation} might only be defined after a finite extension of $K$. In this case, our argument gives a perfectoid space over this (necessarily perfectoid) field. We can then use Galois descent to get a perfectoid space over our original field. This uses that the quotient of a perfectoid space by a finite group often remains perfectoid: see Theorem 1.4 of \cite{Hansen quotients} for details. One must also check that Galois descent commutes with tilde-limits: this is true, but we omit the proof as we never use it. 
	\end{remark}
