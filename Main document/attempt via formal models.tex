...
This work started as a project group at the Arizona Winter School 2017, and we would like to thank everyone involved for organising such a great conference. In particular, we would like to thank Bhargav Bhatt for setting the project on which this article is based, for his guidance and for his constant encouragement. 
...
%%%%%%%%%%%%	
	...
	In characteristic $0$ one can now argue that relative to $A$ this is a pro-\'etale object over the pro-\'etale perfectoid object $E/M^{1/p^\infty}$, and thus again perfectoid by Lemma~4.6 of \cite{p-adic Hodge}. But due to the lemmas, one can give a more elementary argument, that also says something about the local geometry of the limit: 
	\begin{lemma}
	...
	There exists a perfectoid space $(E/M^{1/p^\infty})_\infty$ such that $(E/M^{1/p^\infty})_\infty \sim \varprojlim_{v}(E/M^{1/p^\infty})$. Moreover, the projection map $\pi:(E/M^{1/p^\infty})_\infty\rightarrow E/M^{1/p^\infty}$ is an infinite covering map, that is every point of $E/M^{1/p^\infty}$ has an open neighbourhood $U$ such that $\pi^{-1}(U)$ is isomorphic to $D_\infty \times U$,  a disjoint union of infinitely many copies of $U$.
	\end{proposition}
	\begin{proof}
