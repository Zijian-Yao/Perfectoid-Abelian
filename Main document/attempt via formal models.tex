...
\begin{document}
	...
	We briefly recall how $\mathbb G_m^{\operatorname{an}}$ is constructed: The following is inspired by \cite{Bosch lectures}, \S 9.2, although we choose slightly different constructions. Let $q \in K^\times$ with $|q|\leq 1$. Consider the annulus $\mathcal B(q,1)$ of radii $|q|$ and $1$ inside $\mathbb A_K^{\operatorname{an}}$:
	\[\mathcal B(q,1) = \operatorname{Sp}(L_q),\quad \text{ where } L_q = K\langle X,Z\rangle/(XZ-q). \]
	Similarly, for $q\in K^\times$ with $|q|\geq 1$ one constructs the annulus $\mathcal B(1,q)$ by
	\[\mathcal B(1,q) = \operatorname{Sp}(L_{q}),\quad \text{ where } L_{q} = K\langle q^{-1}X,Z\rangle/(XZ-1)\]
	...
	\begin{center}
		...
		\begin{tikzcd}
			\mathcal B(q^{n},1) & \mathcal B(q^{n-1},1) \arrow[l, hook'] & Z \arrow[r, maps to] \arrow[d, maps to] & qZ \arrow[d, maps to] \\
			\mathcal B(q^{n/p},1) \arrow[u, "{[p]}"] & \mathcal B(q^{(n-1)/p},1) \arrow[u, "{[p]}"'] \arrow[l, hook'] & Z^p \arrow[r, maps to] & (q^{1/p}Z)^p=qZ^p.
		\end{tikzcd}
		\end{equation}
	\end{center} 
	...
	\begin{alignat*}{4}
		\mathfrak \mathcal B(q,1)&:=&& \operatorname{Spf}(L_{q}^\circ), &\quad&\mathfrak \mathcal B(q,1)_\eta &=& \mathcal B(q,1)\\ 
		\mathfrak \mathcal B(1,q)&:=&& \operatorname{Spf}(L_{q^{-1}}^\circ), &\quad&\mathfrak \mathcal B(1,q)_\eta &=& \mathcal B(1,q)
	\end{alignat*}
	For the glueing maps~(\ref{torus explicit cover glue map 1}) and (\ref{torus explicit cover glue map 2}) it is clear from $a_n/a_{n-1} = b_{n-1}/b_n = q$ that these extend to glueing maps $\mathfrak \mathcal B(q^n,1)\hookleftarrow \mathcal B(q^{n-1},1)$ and $\mathfrak \mathcal B(1,q^{-n})\hookleftarrow \mathcal B(1,q^{-(n-1)})$. We conclude:

	\begin{lemma}\label{formal model of torus}
		The affine formal schemes $\mathfrak \mathcal B(q^n,1)$ and $\mathfrak \mathcal B(1,q^n)$ glue together to a flat formal scheme $\mathfrak G_q$ such that $(\mathfrak G_q)_\eta = \mathbb G_m^{\operatorname{an}}$. In other words, $\mathfrak G_q$ is a formal model for $\mathbb G_m^{\operatorname{an}}$.
	\end{lemma}
	...
	\[\mathfrak \mathcal B(q,1)\xleftarrow{[p]} \mathfrak \mathcal B(q^{1/p},1)\]
	and similarly for $\mathfrak \mathcal B(1,q^{-1})$. The diagram~(\ref{diagram showing that [p]-multiplication on torus glues together}) shows that these maps glue to a morphism
	...
	\[\mathfrak \mathcal B(a,1)\times \mathfrak \mathcal B(b,1)\rightarrow \mathfrak \mathcal B(ab,1).\]
	This does \textit{not} mean that multiplication has a formal model $\mathfrak G\times \mathfrak G\rightarrow \mathfrak G$. Indeed, the chosen description has different covers on source and target which in the formal case give rise to different formal schemes (the inversion map $i:\mathbb G_m^{\operatorname{an}}\rightarrow \mathbb G_m^{\operatorname{an}}$ on the other hand does have a formal model). Nevertheless, if we take $a=1$ in the above , we see that we do have an action of the torus $\overline{T}:=\mathfrak \mathcal B(1,1)$ on each of $\mathfrak \mathcal B(b,1)$ and $\mathfrak \mathcal B(1,b)$. Using the formal models from the last section, we conclude:
	
	\begin{proposition}\label{action on formal model of torus}
		For any $q\in K^\times$ with $|q|<1$, the formal torus $\overline{T}:=\mathfrak \mathcal B(1,1)$ has a natural action on $\mathfrak G_q$ via a map
		\[\mathfrak m:\overline{T}\times \mathfrak G_q\rightarrow \mathfrak G_q.\]
		This map is a formal model of the action of the annulus $\overline{T}=\mathcal B(1,1)$ on $\mathbb G_m^{\operatorname{an}}$. Furthermore, this action is compatible with the models for $[p]$ in the sense that if there is a $p$-th root $q^{1/p}\in K$, then the following diagram commutes.
		\begin{center}
		\begin{tikzcd}
	...
	\end{equation}
	where $B$ is an abelian variety of good reduction, and $\overline{T}$ is the completion of a torus of rank $r$ over $K$.
	...
	\begin{center}
		\begin{tikzcd}[column sep={1cm,between origins},row sep={1cm,between origins}]
			...
			\tilde{T} \arrow[rrr] \arrow[uuu] \arrow[rru, "\, F"'] &  &  & \tilde{E} \arrow[rru, "\, F"'] \arrow[uuu] &  &  &  & 
				\end{tikzcd}
	\end{center}
	Since the composed maps $\tilde{E}\rightarrow \tilde{E}$ on the bottom left, $\tilde{T}\rightarrow \tilde{T}$ on the bottom right and  $\tilde{\mathfrak T}_{q^{1/p}}\rightarrow \tilde{\mathfrak T}_{q}$ on the upper right by construction are the reductions of the respective $p$-multiplication maps $[p]$, the functoriality of the associated bundle construction in Proposition~\ref{associated bundle construction in the semi-linear case is a sort of fibered bifunctor} implies that the two maps on the upper left compose to the reduction of $[\mathfrak p]\times^{[p]}[p]$. But $[\mathfrak p]\times^{[p]}[p]$ is equal to $[\mathfrak p]:\mathfrak E_{q^{1/p}}\xrightarrow{} \mathfrak E_q$ by definition of the latter. 	This completes the proof that the reduction of $[\mathfrak p]:\mathfrak E_{q^{1/p}}\xrightarrow{} \mathfrak E_q$ factors through the relative Frobenius on $\tilde{\mathfrak E}_{q^{1/p}}$.
	...
	Let $K$ be a complete algebraically closed non-archimedean field with ring of integers $\mathcal O_K$. Let $A$ be an abelian variety over $K$. Recall from Proposition~\ref{Raynaud uniformisation} that associated to $A$ we have a Raynaud extension $E$ which is an extension of a split rigid torus $T$ of rank $r$ by an abelian variety $B$ of good reduction, and there is a lattice $M\subseteq E$ of rank $r$ such that $A=E/M$.
	...
	Note that this map is surjective by our assumption that $K$ is complete algebraically closed.
		...
		The formal torus $\overline{T}$ on $K$-points corresponds to $\overline{T}(K) = (\mathcal O_K^\times)^r$ and is thus in the kernel of $-\log$. As locally on $U\subseteq B$ we have $E|_U = T\times^{\overline{T}}\overline{E}|_{U}$, the map $\ell$ therefore extends to a group homomorphism 
	...
	\[S_r = \{(x_1,\dots,x_r)\in \mathbb R^r | |x_i|\leq d_i \}\]
	...
	\begin{lemma}\label{cube around origin gives local chart for E/M}
		The inverse image $\ell^{-1}(S_r)\subseteq E(K)$ is the underlying set of the admissible open subset $E(q):=\mathcal B(q,q^{-1})\times^{\overline{T}}\overline{E}$.
	\end{lemma}
	\begin{proof}
		One shows this first for the map $T\rightarrow \mathbb R^d$, where it is clear that the preimage is $\mathcal B(q,q^{-1})$. This is also described in \S 6.4 of \cite{FvdP}. The statement for $\mathcal B(q,q^{-1})\times^{\overline{T}}\overline{E}$ follows by direct inspection on local trivialisations $\mathcal B(q,q^{-1})\times U$ for $U\subseteq B$.
	\end{proof}
	
	
	Note that the map $\mathbb R^d\rightarrow \mathbb R^d/\Lambda$ maps $S_r$ bijectively onto its image. 
	Lemma~\ref{cube around origin gives local chart for E/M} says that we can use $\mathcal B(q,q^{-1})\times^{\overline{T}}\overline{E}$ as a chart for $E/M$ around the origin.
	...
	\begin{lemma}\label{cube around point gives local chart for E/M}
		With notation as before, let $s\in \mathbb R^r$ and let $c \in T(K)$ be any point such that $l(c)=s$. Then the inverse image $\ell^{-1}(s+S_r)\subseteq E(K)$ of the translation of $S_r$ by $s$ is the underlying set of the admissible open subset $E(c,q) := (c\cdot \mathcal B(q,q^{-1}))\times^{\overline{T}}\overline{E} \subseteq E$.
	\end{lemma}
	\begin{definition}
		We call spaces of the form $E(c,q)\subseteq E$ \textbf{cuboids} centered at $c$. Note that more precisely they are locally a cuboid   $\mathcal B(q,q^{-1}))\subseteq T$ times an admissible open subset of the abelian variety $B$.
	\end{definition}
	...
	One can reconstruct $A$ from any such cover by glueing $E_1,\dots,E_k$ as follows: By construction, for any $E_i$ the translates $q\cdot E_i$ by $q\in M$ are pairwise disjoint and we thus have a canonical projection $\pi$ from the union $\cup_{m\in M} (m\cdot E_i)\subseteq E$ to $E_i$.	Let $E_{ij}:=\cup_{m\in M} (m\cdot E_i) \cap E_j \subseteq E$. Then we glue $E_j$ to $E_i$ via the map
	...
	\end{lemma} 
	\begin{proof}
	Since $\mathbb R^d/\Lambda$ is proper, we can find finitely many $s_1,\dots,s_k$ such that $\mathbb R^d/\Lambda$ is covered by the $s_i+S$. When we chose $c_1,\dots,c_k \in T(K)$ as in the Lemma, then the corresponding $E_i:=E(c_i,q)$ are an atlas of $A=E/M$ by admissible open subsets of $E$.
	...
	\end{proof}
	Finally, we need some control about what happens to the cubes under $[p]$-multiplication.
	...
	\begin{proof}
		...
		\[[p]\times^{[p]}[p]: T\times^{\overline{T}}\overline{E}\rightarrow T\times^{\overline{T}}\overline{E} \]
		by Lemma~\ref{p-multiplication is induced from Borel construction}. Thus $(c\cdot \mathcal B(q,q^{-1}))\times^{\overline{T}}\overline{E}$ pulls back to $(c^{1/p}\cdot \mathcal B(q^{1/p},q^{-1/p}))\times^{\overline{T}}\overline{E}$.
	\end{proof}

	\subsection{The two towers}
	 We first observe that
		the $[p]$-multiplication of $A$ can be separated into two different towers, using a factorisation of $[p]$ into characteristic $0$-analogues of the Frobenius and Verschiebung morphisms as follows: Choose a $\Gamma_0(p^\infty)$-level structure on $A$. Such a choice is equivalent to a choice of a $\Gamma_0(p^\infty)$-structure on $B$ and a choice of  lattices 
		\[M\subseteq M^{1/p} \subseteq M^{1/p^2}\subseteq \dots \subseteq  E\]
		of rank $r$ for all $n\in \mathbb N$ such that $M^{1/p^n}/M\subseteq E/M$ is a finite order $p^{rn}$-subgroup with underlying group isomorphic to $\mathbb Z/p^n\mathbb Z$. In particular, the multiplication $[p^n]$ sends $M^{1/p^n}$ isomorphically to $M$ and $M^{1/p^n}/M \subseteq A[p^n]$ is an \'etale subgroup. In particular, the rigid quotient $E/M^{1/p^n}$ is another abelian variety and the quotient map $E/M\rightarrow E/M^{1/p^n}$ is a degree-$p^n$ isogeny through which  $[p]:A\rightarrow A$ factors: 
		\begin{center}
		...
		\begin{tikzcd}[column sep={1.2cm,between origins},row sep={0.8cm,between origins}]
			...
			& E/M \arrow[rr,"v"] \arrow[rrdd, "{[p]}"'] &  & E/M^{1/p} \arrow[rr,"v"] \arrow[dd,"f"] &  & E/M^{1/p^2} \arrow[dd,"f"] \\
			&  &  &  &  &  \\
			&  &  & E/M \arrow[rrdd, "{[p]}"'] \arrow[rr,"v"] &  & E/M^{1/p} \arrow[dd,"f"] \\
			&  &  &  &  &  \\.
			&  &  &  &  & E/M
		\end{tikzcd}
		\end{equation}
	...
	\end{proof}
	We also record the following immediate consequence (see Figure~\ref{local-glueing-[p]-tikzpicture} for an illustration).
	
	\begin{lemma}\label{preimage of E_i under p^n is disjoint copies}
		The preimage of $E_i$ under $[p^n]:A\rightarrow A$ is isomorphic to $p^{rm}$ disjoint copies of $E_i^{1/p^n}$. More canonically, we can describe the preimage as the isomorphic image of  $D_n \times E_i^{1/p^n}$ under the multiplication $A\times A\rightarrow A$. The situation is thus as in figure 
	\end{lemma}
	\begin{figure}
	...
	The horizontal maps in diagram~(\ref{p-multiplication tower of E/M splits into vertical and horizontal tower}) induce a natural finite \'etale morphisms $v:E/M^{1/p^\infty}\rightarrow E/M^{1/p^\infty}$ that fit into a Cartesian diagrams
	\begin{center}
		\begin{tikzcd}
			E/M^{1/p^\infty} \arrow[d, "{}"] \arrow[r,"v^m"] & E/M^{1/p^\infty} \arrow[d, "{}"] \\
			E/M^{1/p^{n-m}} \arrow[r,"v"] & E/M^{1/p^{n}}
		\end{tikzcd}
	\end{center}
	In particular, the preimage of $E_i^{1/p^\infty}$ under $v^m$ is isomorphic to $p^{rm}$ copies of $E_i^{1/p^\infty}$.
	\end{lemma}
	\begin{proof}
		Since $E/M \rightarrow E/M^{1/p}$ is finite \'etale, the fibre product with $E/M \rightarrow E/M^{1/p}$ exists and is perfectoid by Proposition~7.10 of \cite{perfectoid IHES}.
	...
	\begin{proof}
		...
		\begin{center}
			\begin{tikzcd}[row sep = {1cm,between origins}, column sep = {3cm,between origins}]
				\vdots \arrow[d] & \vdots \arrow[d] & \vdots \arrow[d] \\
		...
		\begin{tikzcd}[row sep = {1.5cm,between origins}, column sep = {2cm,between origins}]
			\vdots \arrow[d,"\cong"'] & \vdots \arrow[d, "{[p]}"] & \vdots \arrow[d] \\
