...
\end{enumerate}
	...
	We now want to explicitly construct a tilde-limit of this tower. Recall from Remark~\ref{remark: Definition of the D_n} the finite \'etale subgroups $D_n=M^{1/p^n}/M\subseteq E/M=A$ of rank $p^{rn}$. Since $K$ is algebraically closed, they are constant and uncanonically isomorphism to $(\underline{\mathbb Z/p^{n}\mathbb Z})^{r}=\operatorname{Spa}(\operatorname{Map}(\mathbb (\mathbb Z/p^n\mathbb Z)^r,K),\operatorname{Map}((\mathbb Z/p^n\mathbb Z)^r,\mathcal O_K))$.
	...
	\begin{lemma}\label{lemma: existence and structure of D_infty}
		There exists a perfectoid space $D_\infty$ such that $D_\infty \sim \varprojlim_{[p]} D_n$. Any choice of compatible isomorphisms $D_n\cong\underline{\mathbb Z/p^{n}\mathbb Z}^{r}$ induces an isomorphism 
	...
	\begin{lemma}\label{lemma: tilde-limits of D_n times X_n}
		Let $(X_n)_{n\in \mathbb N}$ be an inverse system of adic spaces with quasicompact and quasiseparated transition maps over $(K,\mathcal O_K)$ and let $X$ be a perfectoid space such that $X\sim \varprojlim_n X_n$. Then $D_\infty \times X$ is a perfectoid tilde-limit
\[D_\infty \times X \sim \varprojlim_{n\in \mathbb N}(D_n\times X_n).\]
	\end{lemma}
	\begin{proof}
		Since $D_\infty$ is perfectoid, the fibre product $D_\infty\times X$ exists and is perfectoid by \cite{perfectoid}, Proposition~6.18. Since the $D_n$ are just a disjoint union of copies of $\operatorname{Spa}(K,\mathcal O_K)$, the fibre product $D_n\times X_n$ exists as an adic space. Let us choose compatible isomorphisms $D_n\cong (\underline{\mathbb Z/p^n\mathbb Z})^r$, then locally on an affinoid open $\operatorname{Spa}(A_n,A_n^{+})\subseteq X_n$, the fibre product is of the form 
		\[D_n\times \operatorname{Spa}(A_n,A_n^{+}) = \operatorname{Spa}(\operatorname{Map}( (\mathbb Z/p^n\mathbb Z)^r,A_n),\operatorname{Map}((\mathbb Z/p^n\mathbb Z)^r,A_n^+)).\]
		Since $X\sim \varprojlim_n X_n$, we may cover $X$ by open affinoids $U=\operatorname{Spa}(A,A^{+})$ such that 
		\[\varinjlim_{\operatorname{Spa}(A_{j},A_{j}^{+})\subseteq X_n} A_{j} \rightarrow A \]
		has dense image. Here the direct limit runs through all affinoid open subspaces $\operatorname{Spa}(A_{j},A_{j}^{+})\subseteq X_n$ for all $n$ through which $U\subseteq X\rightarrow X_n$ factors, as in the definition of tilde-limits. Then by Lemma~\ref{lemma: existence and structure of D_infty} and the construction of products of affinoid perfectoid spaces, we have
		...
		\[\varinjlim_{n} \operatorname{Map}((\mathbb Z/p^n\mathbb Z)^r,A_j)=\operatorname{Map}_{lc}(\mathbb Z_p^r,A_j),\] where the right hand side denotes locally constant morphisms. But it then follows from a pointwise approximation argument that $\varinjlim_j\operatorname{Map}_{lc}(\mathbb Z_p^r,A_j)$ has dense image in $\operatorname{Map}_{lc}(\mathbb Z_p^r,A)$. It is then clear that the latter has dense image in $\operatorname{Map}_{cts}(\mathbb Z_p^r,A)$. We conclude that $D_\infty\times X \sim \varprojlim D_n\times X_n$.
	\end{proof}
	...
	\end{proposition}
	\begin{proof}
	By Lemma~\ref{horizontal etale map pulls back to vertical limit}, the preimage of $E_i^{1/p^\infty}$ under $v^m$ of $E/M^{1/p^\infty}$ is isomorphic to $D_m\times E_i^{1/p^\infty}$. By Lemma~\ref{lemma: tilde-limits of D_n times X_n} we then have
	\[D_\infty \times E_i^{1/p^\infty}\sim \varprojlim_m D_m\times E_i^{1/p^\infty}.\]
	Alternatively, we could without loss of generality assume that $E_i^{1/p^\infty}$ is affinoid perfectoid, and use Proposition~\ref{proposition: tilde-limits of perfectoid spaces} to see that a perfectoid tilde-limit exists. Using Proposition~\ref{SW Proposition 2.4.5} and the universal property of the product, on then obtains an isomorphism of this tilde-limit to $D_\infty\times E_i^{1/p^\infty}$.
	...
	By Lemma~\ref{lemma: tilde-limits of D_n times X_n} this inverse system has perfectoid tilde-limit $D_\infty \times E_i^{1/p^\infty}$. These local tilde-limits glue together to give the desired tilde-limit $A_\infty$.
