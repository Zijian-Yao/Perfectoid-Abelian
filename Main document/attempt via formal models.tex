...
Let $p$ be a prime and let $K$ be a non-archimedean field which is $p$-adic or of characteristic $p$. 
Let us moreover assume that $K$ is algebraically closed. 
For an abelian variety $A$ over $K$ let us consider the inverse system of $A$ under the multiplication by $p$ morphism:
...
The Theorem in the case of abelian varieties $A$ of good reduction appeared already in \cite{Pilloni-Stroh} (Lemme A.16). In order to motivate our strategy for the general case, let us have a look at the proof in the case of good reduction as a motivating example (this example is taken from Exercise 4 -- 6 in \cite{Bhatt} and spelt out in detail in \S \ref{subsection:perfectoid_tilde_limit}, Corollary \ref{tilde-limit exists and is perfectoid in the good reduction case} below):
...
In this case, Raynaud's theory provides us with a short exact sequence 
...
\begin{enumerate}
\item Use formal models to show that there exists a perfectoid tilde-limit $T_\infty$ of $\varprojlim_{[p]} T$;
\item Use the formal models from (1) to construct a perfectoid tilde-limit $E_\infty$ of $\varprojlim_{[p]} E$;
\item Study the projection $E\rightarrow A$ in the limit over $[p]$ to construct the desired space $A_\infty$.
\end{enumerate}

More precisely, this article is organised as follows: In \S2 we develop a notion of a $[p]$-$F$-formal tower for a rigid group $G$ over $K$, which is roughly an inverse system of formal models for $[p]:G\rightarrow G$ that factor through the relative Frobenius mod $p$. This is an axiomatisation of the data that one uses in the construction of $A_\infty$ in the case of good reduction. In particular, the same proof shows that if $G$ admits $[p]$-$F$-formal tower, then there exists a unique perfectoid tilde-limit $G_\infty \sim  \varprojlim_{[p]} G$.
...
\end{comment} 
 ...
 {\color{red} Other grants? Other people?}
