...
\begin{document}
	...
	\begin{abstract}
For an abelian variety $A$ over an algebraically closed non-archimedean field which is either $p$-adic or characteristic $p$, we show that the inverse limit of $A$ under multiplication by $[p]$ carries a natural structure of perfectoid spaces. 
	\end{abstract}
...
{\color{red} We could have used the notion in Scholze's thesis. Explain why we choose to work with this slightly more general notion. }
\end{remark*}
...
Let $\mathcal A$ be a formal abelian scheme over $\mathcal O_K$ with generic fiber $A$. Then on the mod $p$ special fiber $\mathcal A_s = \mathcal A \times \mathcal O_K/p$,  multiplication by $p$ factors through the relative Frobenius map, forcing the inverse limit $ \varprojlim_{[p]} \mathcal A_s $ (which exists in the category of schemes) to be relatively perfect over $\mathcal O_K/p$. Let $\mathcal A_{\infty} = \varprojlim_{[p]} \mathcal A$ (which exists by Lemma \ref{lemma:inverse_limit_formal}), then its adic generic fiber $A_\infty$ is a tilde-limit of the multiplication by $p$ system of $A$, and is perfectoid.  {\color{red} probably say a little more?}
\end{remark*}
...
\addtocontents{toc}{\protect\setcounter{tocdepth}{2}} % end hack
	...
	We denote by $\tilde{X}=\mathfrak X\times_{\operatorname{Spf}\mathcal O_K}\operatorname{Spf}\mathcal O_K/\pi$ its special fibre, considered as a scheme over $\operatorname{Spec}\mathcal O_K/\pi$. 
...
\end{enumerate}
	...
	\begin{definition}
		Let $G$ be a rigid group. A \textbf{$[p]$-formal tower} for $G$ is  a family of formal models $\{\mathfrak G_n\}_{n\in \mathbb N}$ of $G$, together with affine transition maps $[\mathfrak p]_n:\mathfrak G_{n}\rightarrow \mathfrak G_{n-1}$ which are formal models of $[p]:G\rightarrow G$. 
	\end{definition}
	
	More generally, let $U\subseteq G$ be an admissible open subset, a $[p]$-formal tower for $U \subseteq G$ is a formal model tower for the pullback 
	\begin{center}
		\begin{tikzcd}
...
In this subsection we define a formalism to address part (2) of Question \ref{question_intro}
In light of Remark \ref{remark:good_reduction} in the introduction we make the following definition
	
	\begin{definition}
		For a rigid analytic group $G$, we call \textbf{$[p]$-$F$-formal tower} the data of:
		\begin{enumerate}
	...
	%Let $\mathfrak G$ be a flat commutative formal group scheme of topologically finite type over $\mathcal O_K$ for which the p-multiplication map $[p]:\mathfrak G\rightarrow \mathfrak G$ is affine. Let $G = \mathfrak G_\eta$ be the rigid group obtained on the generic fibre. Then $G_\infty$ exists and is perfectoid.
	...
	\begin{example}
		If $\mathfrak G$ is a flat commutative formal group scheme such that $p$-multiplication is affine, then setting $\mathfrak G_n = \mathfrak G$ and taking for $[\mathfrak p]$ the actual $p$-multiplication maps $[p]$ defines a $[p]$-$F$-formal tower for the rigid analytic group $G=\mathfrak G_\eta$.
	\end{example}
...
%%%%%%%%%%%%%%%%%
	...
	\begin{proof}
		...
		Like in Remark~\ref{projective limits of perfectoid spaces as adic spaces}, we then obtain a perfectoid space $D_\infty$. More explicitly, since  the $D_n$ is isomorphic to $\underline {\mathbb Z_p/p^{n}\mathbb Z}$, we can write \[D_\infty \cong\operatorname{Spa}(\operatorname{Map}_{\operatorname{cts}}(\mathbb Z_p,K),\operatorname{Map}_{\operatorname{cts}}(\mathbb Z_p,\mathcal O_K)).\]
	\end{proof}
	...
	Recall that by construction of $(E/M^{1/p^\infty})$ we have a cover of $E/M$ by open subsets $E_i$ that pull back to perfectoid open subspaces $E_i^{1/p^\infty}$ for which $E_i^{1/p^\infty}\sim \varprojlim E_i^{1/p^n}$.
	 ...
	 To see this, cover $E_i^{1/p^\infty}$ by open affinoids $U=\Spa(A,A^{+})$ such that 
	 \[\varinjlim_{\Spa(A_{j},A_{j}^{+})\subseteq E_i^{1/p^n}} A_{j} \rightarrow A \]
	 has dense image, where the direct limit runs through all affinoid open subspaces $\Spa(A_{j},A_{j}^{+})\subseteq E_i^{1/p^n}$ for all $n$ through which $U\subseteq E_i^{1/p^\infty}\rightarrow E_i^{1/p^n}$ factors, as in the definition of tilde limits. Then by the construction of products of affinoid perfectoid spaces, the pullback of $U$ to $(E/M^{1/p^\infty})_\infty$ is 
	\[D_\infty\times U \cong \operatorname{Spa}(\operatorname{Map}_{cts}(\mathbb Z_p,A),\operatorname{Map}_{cts}(\mathbb Z_p,A^+))\]
	 It thus suffices to show that $\varinjlim \operatorname{Map}(\mathbb Z/p^n\mathbb Z,A_{j})$ is dense in $\operatorname{Map}_{cts}(\mathbb Z_p,A)$.
	 As a first step, even though in the above limit $n$ depends on $j$, we may write this as two separate limits, 
	 \[\varinjlim \operatorname{Map}(\mathbb Z/p^n,\mathbb Z,A_{j}) = \varinjlim_{j} \varinjlim_n \operatorname{Map}(\mathbb Z/p^n\mathbb Z,A_{j}).\]
	 For fixed $j$, we then see that 
	 \[\varinjlim_{n} \operatorname{Map}(\mathbb Z/p^n\mathbb Z,A_m)=\operatorname{Map}_{lc}(\mathbb Z_p,A_m)\] where the right hand side denotes locally constant morphisms. But it then follows from a pointwise approximation argument that $\varinjlim\operatorname{Map}_{lc}(\mathbb Z_p,A_m)$ has dense image in $\operatorname{Map}_{lc}(\mathbb Z_p,A)$. It is then clear that the latter has dense image in $\operatorname{Map}_{cts}(\mathbb Z_p,A)$. We conclude that $D_\infty\times E_i^{1/p^\infty} \sim \varprojlim D_n\times E_i^{1/p^n}$.
	 
	That $A_\infty$ is independent of the $\Gamma_0(p^\infty)$-structure up to unique isomorphism is a consequence of the universal property of the perfectoid tilde-limit. To see that $D_\infty$ is a closed subgroup of $A_\infty$, choose $i$ such that the origin of $E/M$ lies in $E_i$. Then the unit section $\Spa(K,\mathcal O_K)\rightarrow E_i^{1/p^\infty}$ induces a closed immersion $D_\infty\hookrightarrow D_\infty \times E_i^{1/p^\infty}\hookrightarrow A_\infty$.
	\end{proof}
	...
	\begin{proof}
		...
		\begin{center}
		\begin{tikzcd}[row sep = {1.2cm,between origins}, column sep = {2cm,between origins}]
		...
		\begin{center}
			\begin{tikzcd}[row sep = {1.2cm,between origins}, column sep = {1.2cm,between origins}]
