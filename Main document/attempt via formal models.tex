...
Let us consider the motivating example of the case of good reduction. This example is generalized from Exercise 4 -- 6 in \cite{Bhatt} and spelt out in details in Subsection \ref{subsection:perfectoid_tilde_limit}. 
...
\numberwithin{theorem}{section}
	...
	Let $G$ be a rigid group over $K$, that is, group object in the category of rigid spaces over $K$.
...
\end{enumerate}
		...
		\subsection{Existence of tilde-limits via formal models} 
  
  
  
	Inverse limits often don't exist in the category of adic spaces (or even rigid spaces). However, we observe the  following lemma:
	...
	Here we follow the following standard terminology:
	\begin{definition} \indent 
	
		\begin{enumerate}
			\item Let $\mathcal X$ be a rigid space over $K$. A \textbf{formal model} of $\mathcal X$ is an admissible topologically finite type formal scheme $\mathfrak X$ over $\mathcal O_K$ together with an isomorphism of its generic fibre $\mathfrak X_\eta \xrightarrow{\sim} \mathcal X$ (which is often suppressed from notation).
	...
	one can always find formal models of the spaces, and formal models of morphisms exist after formal blow ups of the source:
	...
	We now summarise our discussion in this subsection by the following
	\begin{corollary}
