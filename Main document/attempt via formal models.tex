...
\begin{document}
	...
	\begin{abstract}
For an abelian variety $A$ over an algebraically closed non-archimedean field, we show that there exists a perfectoid space $A_\infty$ such that $A_{\infty}\sim\varprojlim_{[p]}A$. 
	\end{abstract}
...
\addtocontents{toc}{\protect\setcounter{tocdepth}{2}} % end hack
	Let $p$ be a prime. Let $K$ be a perfectoid field over $\mathbb Q_p$, %that is either $p$-adic or of characteristic $p$. (I don't think we needed char p fields, also does prop 2.8 make sense in char p?) 
with the ring of integers  $\mathcal O_K$  and  a fixed pseudo-uniformiser $\pi\in \mathcal O_K$.
	By adic spaces over $\operatorname{Spa}(K,\mathcal O_K)$, we mean honest adic spaces in the sense of \cite{SW}, and we adopt the notion of perfectoid spaces defined in \S2 \textit{\small ibid}.  
	
	Throughout the article, we make no distinction between rigid spaces and their corresponding adic spaces. If a rigid space is obtained from a $K$-scheme $X$ via rigid-analytification $X\mapsto X^{\operatorname{an}}$, we will often denote both by the same symbol $X$.
...
\end{enumerate}
	...
	\end{definition}
	More generally, by a $[p]$-formal tower of an admissible open subset $U\subseteq G$ we mean the same formal model data for the tower of pull-backs over $U$
	...
	\subsection{Perfectoidness} 

In light of Remark \ref{remark:good_reduction} we make the following definition
...
%%%%%%%%%%%%	
	...
	\begin{theorem}\label{main theorem}
		Let $A$ be an abelian variety over a complete algebraically closed perfectoid field $K$. Then there is a perfectoid space $A_\infty$ such that
