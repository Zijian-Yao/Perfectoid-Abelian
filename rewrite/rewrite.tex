...
making $E\rightarrow E/M=A$ a rigid group homomorphism. 
	...
	\end{definition}
	In particular, any Raynaud extension is an analytic-locally split short exact sequence.  This stays true after applying $\varprojlim_{[p]}$, as we shall now discuss:
	\begin{proposition}
	...
	\begin{proof}
		...
		and similarly for $\overline{E}$. Here the limit $\varprojlim_F$ amounts to the perfection functor, which exists more generally on formal schemes over $\O_K$. For this reason, we see that by functoriality, the sequence
		\[0\to \overline{T}^{\perf}\to \overline{E}^{\perf}\to \overline{B}^{\perf}\to 0 \]
		is again formal-locally split, and thus exact. Note that $ \overline{T}^{\perf}=\varprojlim_{[p]}\overline{T}=\overline{T}_\infty$. Consequently, we obtain a diagram
		\begin{center}
			\begin{tikzcd}
		...
		To deduce the case of characteristic $0$, it suffices to prove that the desired sequence is the untilt of a sequence in characteristic $p$: For this, we first take any Raynaud extension $0\to \overline{T}'\to \overline{E}'\to \overline{B}'\to 0$ over $\O_{K^{\flat}}$ such that over $\O_{K^{\flat}}/t=\O_K/p$, the two sequences agree. Then in particular, we have an identification
		\begin{center}
			\begin{tikzcd}
			...
			\end{tikzcd}
		\end{center}
		It now follows from the tilting equivalence that the sequence stated in the Proposition is analytic-locally split exact, as desired.
	\end{proof}
	
	
	 We can use this to prove the main result of this section, namely that $E_\infty$ is perfectoid. More generally:
	...
	We can split $[p^n]:E\to E$ into two morphisms as follows:
			...
			For varying $n$, we can thus split the $[p]$-multiplication tower into two towers
			\begin{center}
			\begin{tikzcd}[row sep={1.5cm,between origins},column sep={2.5cm,between origins}]
			...
			We now first take the limit over the vertical tower on the right: Let $\mathfrak U$ be a cover of $B$ of opens $U$ over which $E\to B$ is split, and let $U_n$ be the pullback of along $[p^n]$. Then by the universal property of the fibre product, the section of $E|_U=T\times U$ induces a section of  $E\times_{B,[p^n]}B|_{U_n}\to U_n$ that fits into a commutative diagram
			\begin{center}
			\begin{tikzcd}
			...
			Since locally over $U$, we have 
			\[ E\times_{B}B_\infty|_{U_\infty}=T\times U_\infty \sim \varprojlim T\times U_n,\]
			we moreover have $E\times_{B}B_\infty\sim E\times_{B,[p^n]}B$.
			
			We now \textit{define} $E_\infty$ to be the change-of-fibre of $\overline{E}_{\eta,\infty}$ along the morphism $\overline{T}_{\eta,\infty}\to T_\infty$, i.e.
			\[E_\infty:=T_\infty\times^{\overline{T}_\infty}\overline{E}_\infty, \]
			this is well-defined by the last Proposition.
			It fits into an inverse system
			\begin{center}
				\begin{tikzcd}
			...
			These sequences are now again locally compatibly split, but for a different reason: These sequences are all push-outs for the various projections $T_\infty=\varprojlim_{[p]}T\to T$. Since we know that the sequence on top is analytic-locally split, we get compatible splittings at each level. After refining $\mathfrak U$, we can assume that these splitting exist over the opens $U_\infty$ from above. Like before this give a tilde-limit relation
			\[E_\infty\sim \varprojlim_{[p]\times \id} E\times_BB_\infty.\]
			We now conclude since a horizontal tilde-limit of vertical tilde-limits is a diagonal tilde-limit, as long as the affine opens on which the density property holds can be taken compatibly: This follows from pointwise sequential approximation.
	\end{proof}
	...
	\begin{remark}
		...
		However, this construction does not give the local splittings in Proposition 3.9.
	\end{remark}
