...
\end{lemma}
	...
	\begin{corollary}\label{c:universal-covers-of-curves}
		\begin{enumerate}
		\item There is a perfectoid tilde-limit $\tilde{C} \sim \varprojlim_{G}C_H$ where $H$ ranges over the open subgroups of $ \pi_1(C,x)$. 
		...
		\[ X = \tilde{C}\times^{\pi_1(C,x)}F(X):=(\tilde{C}\times\underline{F(X)})/\pi_1(C,x).\]
	\end{enumerate}
	\end{corollary}
\begin{remark}
