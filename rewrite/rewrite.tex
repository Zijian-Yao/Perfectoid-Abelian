...
As a third application, we describe how one can obtain functorial perfectoid pro-\'etale covers of curves over $K$. This was observed by Hansen \cite{Hansen-blog}.
	...
	\begin{corollary}
		There is a perfectoid tilde-limit $C_\infty \sim \varprojlim_{G}C_G$ where $G$ ranges over the open subgroups of $ \pi_1(C,x)$. This space is a pro-\'etale $\pi_1(C,x)$-torsor over $C$, a pro-universal object for finite \'etale covers of $C$, and a universal object for perfectoid pro-\'etale covers of $C$.
	\end{corollary}
...
For every open subgroup $H$ of $\pi_1(C,x)$, $C_H\rightarrow C$ is an $H$-torsor, so we get compatible isomorphisms $C_H\times_C C_H\cong H\times C_H$. By the universal property of perfectoid tilde-limits, these extend to an isomorphism $C_\infty\times_C C_\infty\cong \pi_1(C,x)\times C_\infty$, so $C_\infty$ is a pro-\'etale $\pi_1(C,x)$-torsor over $C$. (Note that $C_\infty\times_C C_\infty$ is a pro-\'etale cover of the perfectoid space $C_\infty$, and therefore exists and is perfectoid.) This is a pro-universal object for finite \'etale covers of $C$ by the definition of tilde-limits: any finite \'etale cover $C_H\rightarrow C$ is in the inverse system defining $C_\infty$, so the map $C_\infty\rightarrow C$ factors through $C_H$. The universality of $C_\infty$ among perfectoid pro-\'etale tilde-limits of $C$ follows from the same argument as was used to construct the map $C_\infty\rightarrow C'_\infty$ above.
