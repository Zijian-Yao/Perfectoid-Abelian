...
As observed by Hansen, one can deduce from Theorem~\ref{thm:main_thm_intro} the existence of certain universal covers of curves by embedding them into their Jacobian:
	...
	Let $C$ be a connected smooth projective curve of genus $g\geq 1$ over $K$, considered as an analytic adic space. Fix a geometric base point $x:\Spa(K)\to C$ and consider the pro-\'etale fundamental group $\pi_1(C,x)$.
	\begin{enumerate}
...
Second, we note that the analogue of this corollary also works for $C$ replaced by an abelian variety, in which case the pro-\'etale fundamental group is isomorphic to the absolute Tate module $TA:=\varprojlim_{N\in \N}A[N]$. In particular, one obtains from this two different natural ways to uniformise the diamond $A^{\diamond}$ attached to $A$: First, as an immediate consequence of Theorem~\ref{thm:main_thm_intro}, we can write
\[ A^{\diamond}=A_\infty/T_pA.\]
Second, one also obtains a perfectoid tilde-limit $\tilde{A}\sim \varprojlim_{[N]} A$ together with a natural isomorphism
...
\begin{corollary}\label{c:universal-covers-of-abelian-varieties}
	Let $A$ be an abelian variety over $K$, then there is a perfectoid tilde-limit $\tilde{A}\sim \varprojlim_{[N]}A$ and the analogous statements of Corollary~\ref{c:universal-covers-of-abelian-varieties}.2 and 3 hold for the $TA$-torsor $\tilde{A}\to A$. In particular, we also have a natural uniformisation
