...
\end{tikzcd}.\]
	...
	The morphism $[p^n]:E\to E$ induces a morphism of short exact sequences of Raynaud extensions. When we consider $E$ as an extension of $B$ by $T$, this means more precisely that we can split up $[p^n]:E\to E$ into two morphisms of short exact sequences
			...
			The basic idea is now to trace local splittings of $\pi$ through the diagram: Let $\mathfrak U$ be a cover of $B$ of opens $U$ over which $E\to B$ is split. Since $E$ arises via pushout from the formal model $\overline{E}$, such a cover already exists in the formal topology on $B$. Let $U_n$ be the pullback of $U$ along $[p^n]$. 
		We now use the following Lemma:
		\begin{lemma}
			For any $U\in \mathfrak U$, the morphism $\pi:E\to B$ is still split over $U_n=[p^n]^{-1}(U)$.
		\end{lemma}
		\begin{proof}
			We may reduce to the case of $n=1$.
			Since $\mathfrak U$ is by assumption a formal open cover, it suffices to prove this for the morphism of formal schemes $\overline{E}\to \overline{B}$.
			
			We first claim that the reduction $\overline{E}/p\to \overline{B}/p$ over $\O_K/p$ admits a splitting over $U_n/p$ (here we emphasize that we really mean $p$, not $\varpi$: in particular, if $K$ has characteristic $p$, we have $\O_K/p=\O_K$). To see this, we use that in characteristic $p$, the morphism $[p]:\overline{B}/p\to \overline{B}/p$ decomposes into the Verschiebung $V:(\overline{B}/p)^{(p)}\to \overline{B}/p$ and the Frobenius morphism $F:\overline{B}/p\to (\overline{B}/p)^{(p)}$.
			...
			\begin{center}
				...
				\end{tikzcd}
			\end{center}
			The splitting over $U/p$ in the bottom row lifts to a splitting in the middle row by the universal property of the fibre product, since the middle row is obtained from the bottom row via pullback along $V:(\overline{B}/p)^{(p)}\to \overline{B}/p$.  The splitting can then be further lifted to the top row since by functoriality of the relative Frobenius morphism $F$. This shows that $\overline{E}/p\to \overline{B}/p$ is indeed split over $[p]^{-1}(U/p)=[p]^{-1}(U)/p=U_n/p$.
			
			It remains to lift this splitting to $\pi:\overline{E}\to \overline{B}$. For this  we use that $\pi$ is formally smooth, and thus we get the desired lift of the splitting from the formal lifting diagram
			\[\begin{tikzcd}
			...
			\end{tikzcd}\]
		This shows that $\overline{E}\to \overline{B}$ is split over $U_n$, and thus so is $E\to B$.
		\end{proof}
		We conclude from the Lemma that locally over $U$, the diagram~\eqref{eq:split-diag} is of the form
		\begin{center}
			\begin{tikzcd}[row sep = {1.2cm,between origins}]
			...
			\end{tikzcd}
		\end{center}
	Let $U_\infty\sim \varprojlim U_n$ be the perfectoid tilde-limit that exists by Lemma~\ref{SW Proposition 2.4.3}. Then by Lemma~\ref{affinoid tilde-limits commute with fibre products}, the system in the middle has perfectoid tilde-limit $T_\infty\times U_\infty$. Glueing these for all $U\in\mathfrak U$ produces the desired space $E_\infty\sim \varprojlim_{[p]} E$ which by construction fits into an analytic-locally split short exact sequence
	\[0\to T_\infty\to E_\infty\to B_\infty\to 0 \]
	as desired.
	\end{proof}
