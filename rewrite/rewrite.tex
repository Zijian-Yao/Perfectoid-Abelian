...
\end{tikzcd}.\]
	...
	The morphism $[p^n]:E\to E$ induces a morphism of short exact sequences
		...
		\begin{proof}
			...
			Since $\mathfrak U$ is by assumption a formal open cover, it suffices to prove the Lemma for the morphism of formal schemes $\overline{E}\to \overline{B}$.
			...
			It remains to lift this splitting from the special fibre over $\O_K/s$ to $\O_K$. For this  we use that $\pi:\overline{E}\to \overline{B}$ is formally smooth. The formal lifting diagram
			\[\begin{tikzcd}
				\overline E_{U_n} \arrow[d, "\pi"] & \overline E_{|U_n}/s \arrow[r] \arrow[l] &  U_n/s \arrow[l, bend right] \arrow[d]\\
				U_n \arrow[rr,equal]                     &                                          & U_n. \arrow[llu, dotted]
			\end{tikzcd}\]
		therefore produces the splitting $U_n\to \overline{E}$. In particular, $\overline{E}\to \overline{B}$ is split over $U_n$.
		\end{proof}
