...
\end{remark}
\begin{example}
If $C=E$ is an elliptic curve, then $\pi_1(C,x)=\varprojlim_{N\in \N} E[N](K)$, and thus \[\tilde{E}\sim\varprojlim_{[N]}E\] is the tilde-limit over multiplication by $N$ for all $N\in \N$, in contrast to $E_\infty\sim \varprojlim_{[p]} E$.
\end{example}
\begin{proof}
