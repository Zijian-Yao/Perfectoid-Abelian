...
\end{corollary}
\begin{proof}
	Since $S$ is compact, $\mathrm{Map}_{\mathrm{cts}}(S,K)=\mathrm{Map}_{\mathrm{cts}}(S,\O_K)[\tfrac{1}{\pi}]$. Perfectoidness now follows from $\mathrm{Map}_{\mathrm{cts}}(S,\O_K)/\pi=\mathrm{Map}_{\mathrm{lc}}(S,\O_K/\pi)$. The tilde-limit property follows from \ref{SW Proposition 2.4.2}.
\end{proof}
...
\end{proposition}
\begin{proof}
	These are all consequences of the universal property of the perfectoid tilde-limit, Prop.~\ref{SW Proposition 2.4.5}, which shows that one can argue like in the case of categorical limits.
\end{proof}
...
We now introduce the formalism of $[p]$-$F$-formal towers to axiomatise the approach mentioned in the introduction giving perfectoid tilde-limits in the case of good reduction.
	...
	\begin{proof}
		...
		To see that $G_\infty$ is perfectoid, we proceed as the proof of \cite{torsion}, Corollary III.2.19. It suffices to prove that $\mathfrak G_\infty = \varprojlim_{[\mathfrak p]} \mathfrak G$ can be covered by formal schemes of the form $\operatorname{Spf}(S)$ where $S$ is a flat $\mathcal O_K$-algebra such that the Frobenius \[S/\pi^{1/p} \rightarrow  S/\pi\] is an isomorphism. Lemma~5.6 of \cite{perfectoid} then shows that $S[1/\pi]$ is perfectoid.
...
%%%%%%%%%%%%	
	...
	We have natural open immersions $\mathcal B(a,a)\hookrightarrow B(a,b)$ and $\mathcal B(b,b)\hookrightarrow B(a,b)$, which we regard as the inner and outer ``boundaries''. Explicitly, \[\mathcal B(a,a)=\mathcal B(a,b)(|\tfrac{a}{X}|\geq 1),\quad \mathcal B(b,b)=\mathcal B(a,b)(|\tfrac{X}{b}|\geq 1).\]
	
	
	
	For any choice of $q\in K^\times$ with $|q|<1$, we can construct $\mathbb G_m^{\mathrm{an}}$ by glueing the annuli $\{\mathcal B(q^i,q^{i-1})\}_{i\in\mathbb{Z}}$ along the inner and outer boundary $\mathcal B(q^i,q^i)$ of $\mathcal  B(q^{i+1},q^{i})$ and $\mathcal B(q^{i}, q^{i-1})$, respectively.
...
(q^{i/p}/X)^p, (X/q^{(i-1)/p})^p&\mapsfrom&& 	q^i/X, X/q^{i-1}.
	...
	\end{proposition}
	\begin{proof}
		Fix a $q\in K^\times$ with $|q|<1$ along with a compatible system of $p^n$th roots $q^{1/p^n}\in K$. Then this follows from  Prop.~\ref{existence of p-F-formal tower implies perfectoid} as by the Lemma, we have a $[p]$-$F$-formal tower
	...
	By taking products, all of the statements in this section immediately generalise to split tori: 
...
%%%%%%%%%%%%
	...
	$\overline{E}\rightarrow \overline{B}$. The fact that $E$ is obtained from $\overline{E}_\eta$ via push-out along $\overline{T}_\eta\rightarrow T$ can now conveniently be expressed in terms of the associated fibre bundle by saying that $E = T\times^{\overline{T}_\eta}\overline{E}_\eta$ in the sense of Def.~\ref{definition of Borel construction}.
%%%%%%%%%%%%
%%      Section 4.5
%%%%%%%%%%%%	
	...
	\begin{proof}
		...
		Functoriality of the associated fibre bundle construction, Prop.~\ref{associated bundle construction in the semi-linear case is a sort of fibered bifunctor}, applied to the below diagram then gives a natural map $[\mathfrak p]:\mathfrak E_{q^{1/p}}\rightarrow \mathfrak E$ making the diagram commute:
		...
		We conclude from the construction in the proof of Prop.~\ref{associated bundle construction in the semi-linear case is a sort of fibered bifunctor} that the morphism  $[\mathfrak p]:\mathfrak E_{q^{1/p}} \rightarrow  \mathfrak E_{q}$ locally on the target is of the form
	...
	and we can identify $\tilde{\mathfrak T}_{q^{1/p}}^{(p)} = \tilde{\mathfrak T}_{q}$ in the following. The same is true for $\tilde{T}^{(p)} = \tilde{T}$. Prop.~\ref{associated bundle construction in the semi-linear case is a sort of fibered bifunctor} gives us a natural morphism
	...
	\begin{center}
		\begin{tikzcd}[column sep={1cm,between origins},row sep={0.7cm,between origins}]
			...
			\tilde{T} \arrow[rrr] \arrow[uuu] \arrow[rru, "\, F"'] &  &  & \tilde{E} \arrow[rru, "\, F"'] \arrow[uuu] &  &  &  & 
				\end{tikzcd}
	\end{center}
	Since the outer map is the formal model $[\mathfrak p]$ by construction in diagram~\eqref{formal model of p-multiplication cube}, this shows that the reduction of $[\mathfrak p]$ mod $\varpi$ factors through the relative Frobenius, as desired.

	The conclusion that $E_\infty$ is perfectoid then follows from Prop.~\ref{existence of p-F-formal tower implies perfectoid}.
	\end{proof}
...
\end{remark}
	...
	The first row in 4 is part of Prop.~\ref{M_infty->E_infty->A'_infty}. Part of 5 then follows immediately from 4.
	\end{proof}
	\begin{remark}
	...
	\end{corollary}
	\begin{proof}
	By Theorem~\ref{thm:main_thm_intro}, we can apply \cite[Theorem 3.3]{Reineke} to the inverse system $\dots \rightarrow A\xrightarrow{[p]}A$.
	\end{proof}
...
\end{definition}
	...
	\begin{definition}
		Let $g:T'\rightarrow T$ be a homomorphism of formal group schemes. Let $\pi:E\rightarrow B$ be a fibre bundle with fibre $F$ and structure group $T$ and let $\pi':E'\rightarrow B'$ be a fibre bundle with fibre $F'$ and structure group $T'$. Then a $g$-linear morphism of fibre bundles is a commutative diagram
				\begin{equation}\label{dg:def-of-morph-of-fibre-bundles}
		\begin{tikzcd}
		E' \arrow[d] \arrow[d, "f_E"] \arrow[r, "\pi'"] & B' \arrow[d, "f_B"] \\
		E \arrow[r, "\pi"] & B
		\end{tikzcd}
		\end{equation}
		such that $f_E$ is $g$-linear. We abbreviate this by writing $f:E'\rightarrow E$.
	\end{definition}
	For a fixed $f_B:B'\rightarrow B$, one can equivalently characterise a morphism of torsors $f:E'\to E$ over $f_B$ by the data of maps $f_B^{-1}(U_i)\to T'$ on some cover of $B$. Using this description, one sees:
	...
	\begin{lemma}\label{universal property of associated fibre construction in the semilinear case}
		In the context of Prop.~\ref{associated bundle construction in the semi-linear case is a sort of fibered bifunctor}, assume moreover that $F',F$ are formal group schemes and that the respective actions come from group homomorphisms $T'\to F'$ and $T\to F$.
